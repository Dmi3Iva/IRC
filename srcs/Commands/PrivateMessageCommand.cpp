	...
	for (vector<string>::iterator it = receivers.begin(); it < receivers.end(); it++) {
		if (_isChannel(*it)) {
			...
			if (channel != _channelsPtr->end()) {
				if (_isReceiverAlreadyGotMessage(handledReceivers, channel->second->getName())) {
			...
			if (userReceiver) {
				if (_isReceiverAlreadyGotMessage(handledReceivers, userReceiver->getNickname())) {
					sendMessage(user->getFD(), ERR_TOOMANYTARGETS(_serverName, user->getNickname(), *it));
				} else {
					sendMessage(userReceiver->getFD(),
...
bool PrivateMessageCommand::_isReceiverAlreadyGotMessage(list<string>& handledReceivers, string nick)
{
...
void PrivateMessageCommand::_eraseSpacesInFront(string& cmd)
{
	int i = 0;
	while (cmd[i] && cmd[i] == ' ')
		i++;
